...
(function () {
  ...
  window.callGoogleWebApp = async function (event) {
    const dlEvent = 'googleWebApp';
    ...
    try {
      ...
      if (!response.ok) {
        ...
        pushToDataLayer(dlEvent, { status: 'Error' });
      } else {
        try {
          data = await response.json();
          pushToDataLayer(dlEvent, { status: 'OK', ...data });
        } catch (err) {
          data = err.message;
          pushToDataLayer(dlEvent, { status: 'Error', ...data });
        }
      }
    } catch (e) {
